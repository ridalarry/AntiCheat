...
public final class CheatUtil {
	...
	public static boolean cantStandAtBetter(Block block) {
		...
		final boolean overAir1 = (otherBlock.getRelative(BlockFace.DOWN).getType() == Material.AIR)
	...
	public static boolean canStandWithin(Block block) {
		...
		final boolean solid = (block.getType().isSolid()) && (!block.getType().name().toLowerCase().contains("door"))
	...
	public static Vector getRotation(Location one, Location two) {
		...
		final float yaw = (float) (Math.atan2(dz, dx) * 180.0D / 3.141592653589793D) - 90.0F;
		final float pitch = (float) -(Math.atan2(dy, distanceXZ) * 180.0D / 3.141592653589793D);
	...
	public static int getLevelForEnchantment(Player player, String enchantment) {
		try {
			...
			final int j = (arrayOfItemStack = player.getInventory().getArmorContents()).length;
			for (int i = 0; i < j; i++) {
	...
	public static double fixXAxis(double x) {
		double touchedX = x;
		final double rem = touchedX - Math.round(touchedX) + 0.01D;
		if (rem < 0.3D) {
	...
	public static boolean isOnGround(final Location location, final int down) {
		...
		if (fracX < 0.3) {
			...
			return fracZ > 0.7 && BlockUtil.isSolid(world.getBlockAt(blockX, blockY, blockZ + 1));
		}
		return false;
	}
	...
	public static boolean isHoveringOverWater(Location player, int blocks) {
		for (int i = player.getBlockY(); i > player.getBlockY() - blocks; i--) {
			final Block newloc = new Location(player.getWorld(), player.getBlockX(), i, player.getBlockZ()).getBlock();
	...
	public static boolean blocksNearB(final Location loc) {
		boolean nearBlocks = false;
		for (final Block block : BlockUtil.getSurroundingB(loc.getBlock())) {
			if (block.getType() != Material.AIR) {
				nearBlocks = true;
				break;
			}
	...
	public static double[] cursor(Player player, LivingEntity entity) {
		final Location entityLoc = entity.getLocation().add(0.0D, entity.getEyeHeight(), 0.0D);
	...
	public static double getAimbotoffset(Location playerLocLoc, double playerEyeHeight, LivingEntity entity) {
		final Location entityLoc = entity.getLocation().add(0.0D, entity.getEyeHeight(), 0.0D);
		final Location playerLoc = playerLocLoc.add(0.0D, playerEyeHeight, 0.0D);
